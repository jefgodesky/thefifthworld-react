...
DELIMITER //
CREATE DEFINER=`root`@`localhost` PROCEDURE `getPageByID`(IN id INT(11) UNSIGNED)
BEGIN
SELECT * FROM pages p WHERE p.id=id;
END;
//
DELIMITER ;
...
DELIMITER //
CREATE DEFINER=`root`@`localhost` PROCEDURE `getPageByPath`(IN path VARCHAR(256))
BEGIN
SELECT * FROM pages p WHERE p.path=path;
END;
//
DELIMITER ;
