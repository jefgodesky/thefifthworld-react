...
export default class Polycule {
  ...
  /**
   * Add an incident of murder or attempted murder to the polycule's history
   * as well as the personal history of each individual in the polycule and
   * each of the victims.
   * @param report {Object} - An object containing information about the crime,
   *   as generated by the `murder` method.
   */

  recordMurder (report) {
    const year = this.getPresent()
    const crime = report.outcome === 'attempted' ? 'attempted murder' : 'murder'
    const entry = {
      year,
      murderer: report.murderer,
      victims: report.victims,
      attempted: report.attempted,
      tags: [ 'crime', crime ]
    }

    const victimsNotInPolycule = report.victims.filter(p => !this.people.includes(p))
    const attemptedNotInPolycule = report.attempted.filter(p => !this.people.includes(p))
    const people = [ ...this.people, ...victimsNotInPolycule, ...attemptedNotInPolycule ]
    people.forEach(p => p.history.add(entry))
    this.history.add(entry)
  }
